...
class Menu:
    ...
    def creat_buttons(self):
        ...
        self.others_button_rect = generic_button_rect.move(self.rect.height / 2, self.rect.width / 2).inflate(-button_margin,-button_margin)
        ...
        Button(self.others_button_rect, self.buttons, self.menu_surfs['others fg'], self.menu_surfs['others bg'])

    def click(self,mouse_pos,mouse_button):
        for sprite in self.buttons:
            if sprite.rect.collidepoint(mouse_pos):
                if mouse_button[1]:#点击鼠标中键
                    if sprite.items['alt']:
                        sprite.main_active = not sprite.main_active
                    else:
                        sprite.main_active = True
                if mouse_button[2]:#点击鼠标右键
                    sprite.switch()
    ...
    def highlight_indicator(self,index):
        ...
        if EDITOR_DATA[index]['menu'] in ('others fg','others bg'):
            pygame.draw.rect(self.display_surface,BUTTON_LINE_COLOR,self.others_button_rect.inflate(4,4),5,4)
